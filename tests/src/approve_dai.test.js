...
const abi_path = `../${pluginName}/abis/` + DAIContractAddr + '.json';
...
nano_models.forEach(function(model) {
  test('[Nano ' + model.letter + '] Approve DAI', zemu(model, async (sim, eth) => {
  const contract = new ethers.Contract(DAIContractAddr, abi);

  // 50 DAI
  const channelStakeFees = 50;
  const feesAmount = ethers.utils.parseUnits(channelStakeFees.toString(), 18);

  const epnsCore = "0x66329fdd4042928bfcab60b179e1538d56eeeeee";

  const {data} = await contract.populateTransaction.approve(epnsCore, feesAmount);

  // Get the generic transaction template
  let unsignedTx = genericTx;
  // Modify `to` to make it interact with the contract
  unsignedTx.to = DAIContractAddr;
  ...
  await sim.navigateAndCompareSnapshots('.', model.name + '_approve_dai', [right_clicks, 0]);
