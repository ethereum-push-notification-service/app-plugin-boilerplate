...
// EDIT THIS: build your own test
nano_models.forEach(function(model) {
  test('[Nano ' + model.letter + '] Add Delegate', zemu(model, async (sim, eth) => {
  ...
  const {data} = await contract.populateTransaction.addDelegate(delegateeAddress);
  ...
  await sim.navigateAndCompareSnapshots('.', model.name + '_add_delegate', [right_clicks, 0]);
