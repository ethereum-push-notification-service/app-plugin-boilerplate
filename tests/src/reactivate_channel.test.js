...
nano_models.forEach(function(model) {
  test('[Nano ' + model.letter + '] Reactivate Channel', zemu(model, async (sim, eth) => {
  ...
  const {data} = await contract.populateTransaction.reactivateChannel(feesAmount);
  ...
  await sim.navigateAndCompareSnapshots('.', model.name + '_reactivate_channel', [right_clicks, 0]);
