...
const EPNSCommContractAddr = "0xb3971bcef2d791bc4027bbfedfb47319a4aaaaaa";
// EDIT THIS: Replace `boilerplate` with your plugin name
const pluginName = "EPNS";
const abi_path = `../${pluginName}/abis/` + EPNSCommContractAddr + '.json';
...
// EDIT THIS: build your own test
nano_models.forEach(function(model) {
  test('[Nano ' + model.letter + '] Remove Delegate', zemu(model, async (sim, eth) => {
  const contract = new ethers.Contract(EPNSCommContractAddr, abi);

  const delegateeAddress = '0x52f856a160733a860ae7dc98dc71061be33a28b3';

  const {data} = await contract.populateTransaction.removeDelegate(delegateeAddress);

  // Get the generic transaction template
  let unsignedTx = genericTx;
  // Modify `to` to make it interact with the contract
  unsignedTx.to = EPNSCommContractAddr;
  ...
  await sim.navigateAndCompareSnapshots('.', model.name + '_remove_delegate', [right_clicks, 0]);
