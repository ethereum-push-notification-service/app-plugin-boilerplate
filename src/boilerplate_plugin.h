...
typedef enum {
   EPNS_ADD_DELEGATE = 0,
   EPNS_REMOVE_DELEGATE,
   EPNS_REACTIVATE_CHANNEL,
   EPNS_DEACTIVATE_CHANNEL,
   EPNS_CREATE_CHANNEL,
   EPNS_DAI_APPROVE
} selector_t;
...
extern const uint32_t EPNS_SELECTORS[NUM_SELECTORS];
...
typedef struct context_t {
    ...
    uint8_t channel_fees_amount[INT256_LENGTH];
    uint8_t tokens[INT256_LENGTH];
